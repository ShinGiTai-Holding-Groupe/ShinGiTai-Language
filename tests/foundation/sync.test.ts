...
try {
  enqueueCommand([command()], command({ commandId: "command-2", semanticPayloadHash: "different" }));
} catch {
...
assert(resolution.source === "server", "Client clock must never override canonical learning state.");
...
assert(selectDelta(entities, "tenant-1", 5).map((item) => item.entityId).join(",") === "lesson-2", "Delta must be tenant scoped.");
...
assert(result.latestServerRevision === 7 && result.delta.length === 1, "Sync result must use tenant canonical revisions.");
